    ...
    <AdminGuard>
      <div className="max-w-6xl space-y-4">
        <div className="flex items-start justify-between">
          ...
          <Link href="/admin" className="rounded-2xl border border-black/10 px-4 py-2 text-sm font-semibold hover:bg-black/[0.03]">
            ← Back
          </Link>
        </div>
        ...
        <div className="rounded-3xl border border-black/10 bg-white p-5">
                  ...
                  {rows.map((m) => (
                    <tr key={m.user_id} className="border-t border-black/5">
                      ...
                      <td className="py-3 flex gap-2">
                        <button onClick={() => setStatus(m.user_id, "active")} className="rounded-xl bg-emerald-700 px-3 py-2 text-white font-semibold hover:bg-emerald-800">
                          Activate
                        </button>
                        <button onClick={() => setStatus(m.user_id, "suspended")} className="rounded-xl bg-red-600 px-3 py-2 text-white font-semibold hover:bg-red-700">
                          Suspend
                        </button>
                      </td>
                    </tr>
          ...
          <div className="mt-4">
            <button onClick={load} className="rounded-xl border border-black/10 px-4 py-2 text-sm font-semibold hover:bg-black/[0.03]">
              Refresh
            </button>
          </div>
        </div>
