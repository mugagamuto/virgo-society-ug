...
export default function MemberProjectDetail() {
  ...
  async function load() {
    ...
    try {
      // read project directly (if RLS blocks, we can move this to server later)
  ...
  async function uploadDoc(e: React.FormEvent) {
    ...
    try {
      ...
      if (!userId) return setMsg("Please log in again.");
      ...
      const res = await fetch(`/api/members/projects/${encodeURIComponent(id)}/upload-doc`, {
        method: "POST",
        body: form,
      });
  ...
  async function submitForReview() {
    ...
    try {
      ...
      if (!user?.id) return setMsg("Please log in again.");
      ...
      setMsg("Submitted for admin review. Waiting approval.");
      await load();
    } catch (e: any) {
    ...
    <div className="max-w-5xl space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-mutedInk">Upload documents, then submit for admin approval.</div>
          ...
          <div className="rounded-3xl border border-black/10 bg-white p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-black/10 p-4">
                ...
                <div className="mt-1 text-sm font-medium">UGX {Number(project.budget_ugx ?? 0).toLocaleString()}</div>
              </div>
            </div>
